...
describe('Preparación pública', () => {
  it('identifica las secciones públicas sin convertir rutas desconocidas en datos privados', () => {
    ...
    expect(Object.values(pagePaths)).toEqual(['/catalogo', '/coleccion', '/deseos', '/contacto', '/privacidad'])
    expect(pageFromPath('/deseos')).toBe('wishlists')
  ...
  it('publica contacto, derechos, proveedores y límites reales de conservación sin prometer borrado de Gmail', () => {
    ...
    expect(html).toContain('Listas de deseos:')
    expect(html).toContain('sus listas de deseos y las cartas de esas listas')
    expect(html).toContain('La copia JSON incluye únicamente la colección, no las listas de deseos ni sus cartas')
  ...
  it('sirve las rutas de la aplicación sin reescribir la API y excluye colección y deseos de descubrimiento', () => {
    ...
    for (const path of [pagePaths.collection, pagePaths.wishlists]) {
      expect(config.rewrites).toContainEqual({ source: path, destination: '/index.html' })
      expect(config.headers.some((rule: { source: string; headers: { key: string; value: string }[] }) => rule.source === path && rule.headers.some((header) => header.key === 'X-Robots-Tag' && header.value.includes('noindex')))).toBe(true)
