    ...
    <WishlistProvider userId={user?.id ?? null}>
      ...
      <header className="header">
        <div className="header-inner">
          <a 
            href={pagePaths.catalog} 
            className="brand" 
            aria-label="Pokéfolio, inicio" 
            onClick={(event) => { 
          ...
          >
            <span className="brand-mark"><span className="pokeball" aria-hidden="true" /></span>pokéfolio<span className="brand-dot">.</span>
          </a>
          <nav aria-label="Navegación principal">
            <button className={view === 'catalog' ? 'nav-item active' : 'nav-item'} aria-current={view === 'catalog' ? 'page' : undefined} onClick={() => setView('catalog')}><SearchIcon size={17} />Explorar</button>
            <button className={view === 'collection' ? 'nav-item active' : 'nav-item'} aria-current={view === 'collection' ? 'page' : undefined} onClick={() => setView('collection')}><BookOpen size={17} />Mi colección{user && <span className="count-badge">{stats.copies}</span>}</button>
            <button className={view === 'wishlists' ? 'nav-item active' : 'nav-item'} aria-current={view === 'wishlists' ? 'page' : undefined} onClick={() => setView('wishlists')}><Heart size={17} />Deseos</button>
          ...
          <div className="account">
            ...
            {user ? (
              <>
                ...
                <button className="icon-button" aria-label="Cerrar sesión" disabled={busy} onClick={() => void signOut()}><LogOut size={18} /></button>
              </>
            ) : (
              <button className="account-button" disabled={authLoading} onClick={openAuth}><UserRound size={17} /><span>{authLoading ? 'Conectando…' : 'Mi cuenta'}</span></button>
            )}
          </div>
        </div>
      </header>
      <main id="main" className={`main view-${view}`}>
        ...
        {view === 'privacy' ? (
          ...
          <WishlistPage onAuth={openAuth} onOpenCard={(card, language) => setSelection({ card, language })} onExploreCatalog={() => setView('catalog')} />
        ) : view === 'contact' ? (
          <>
            <ContactPage defaultEmail={user?.email ?? ''} />
            <p className="privacy-contact-note">Responsable: PokefolioTCG · Conservación de mensajes: seis meses. <a href={pagePaths.privacy} onClick={(event) => { event.preventDefault(); setView('privacy') }}>Consulta cómo se usan tus datos y ejerce tus derechos</a>.</p>
          </>
        ) : (
