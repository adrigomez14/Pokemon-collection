...
import { collectionStats, createBackupParts, entryValue, euros, formatDate, languages, parseBackup, variants, type Entry, type EntryInput, type Language } from './lib/models'
...
function CollectionApp() {
  ...
  const catalog = useQuery({ queryKey: ['catalog', language, search], queryFn: ({ signal }) => searchCards(language, search, signal), enabled: view === 'catalog' })
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    setSearch({ name: String(data.get('name') ?? ''), set: String(data.get('set') ?? ''), number: String(data.get('number') ?? ''), page: 1 })
  }
  ...
  return <>
    ...
    <main id="main" className="main">
      ...
      <section className="catalog-section">
        ...
        {view === 'catalog' ? <>
          <form key={`${language}:${search.name}`} className="search-panel" onSubmit={submitSearch}><div className="search-main"><SearchIcon size={20} /><input aria-label="Nombre de carta" name="name" placeholder={language === 'ja' ? 'Busca en japonés: ピカチュウ…' : 'Busca por nombre: Pikachu, Charizard…'} defaultValue={search.name} maxLength={100} /><button className="primary" type="submit">Buscar cartas</button></div><div className="search-extras"><label>Expansión (ID)<input name="set" aria-label="Identificador de expansión" placeholder="Ej. sv03.5, base1" defaultValue={search.set} maxLength={100} /></label><label>Número de carta<input name="number" placeholder="Ej. 025" defaultValue={search.number} maxLength={50} /></label><span>Busca en el idioma original de la carta.<br />Imágenes y precios según disponibilidad.</span></div></form>
          ...
          {!visibleEntries.length ? <div className="empty-state"><Layers3 size={35} /><h3>{entries.length ? 'Sin coincidencias' : 'El primer hueco es para tu favorita'}</h3><p>{entries.length ? 'Cambia los filtros para ver más cartas.' : 'Explora el catálogo y añade tu primera carta.'}</p><button className="primary" onClick={() => setView('catalog')}>Explorar cartas</button></div> : <div className="cards-grid">{visibleEntries.map((entry) => <button className="card-tile" disabled={busy} key={entry.id} onClick={() => setSelection({ card: entry.card_snapshot, language: entry.language, entry })}><div className="card-art"><span className="owned-badge">{entry.quantity} × · {entry.condition}</span><CardImage card={entry.card_snapshot} /></div><div className="card-info"><span className="card-code">{entry.card_snapshot.set.name} · {entry.language.toUpperCase()}</span><h3>{entry.card_snapshot.name}</h3><span className="variant-label">{variants[entry.variant]} · N.º {entry.card_snapshot.localId}</span><div className="card-tile-footer"><strong>{euros(entryValue(entry))}</strong><small>{entry.manual_value === null ? 'referencia / ud.' : 'manual / ud.'}</small></div><small className="price-date">{entry.manual_value === null ? `Datos: ${formatDate(entry.card_snapshot.pricing?.cardmarket?.updated)}` : 'Valoración personal'}</small></div></button>)}</div>}
