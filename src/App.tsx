    ...
    <main id="main" className="main">
      ...
      {view === 'contact' ? <ContactPage defaultEmail={user?.email ?? ''} /> : <section className="catalog-section">
        ...
        {view === 'catalog' ? <>
          ...
          {catalog.isPending ? <div className="cards-grid" aria-label="Cargando catálogo" role="status">{Array.from({ length: 8 }, (_, i) => <div className="skeleton" key={i} />)}</div> : catalog.isError ? <div className="empty-state"><CircleHelp /><h3>No se pudo cargar el catálogo</h3><p>{catalog.error.message}</p><button onClick={() => void catalog.refetch()}>Reintentar</button></div> : !catalog.data?.length ? <div className="empty-state"><SearchIcon /><h3>No encontramos esas cartas</h3><p>Revisa el idioma, la escritura del nombre exacto y los filtros combinados. Usa «Limpiar filtros» para empezar de nuevo.</p></div> : <div className={catalogLayout === 'list' ? 'cards-list' : 'cards-grid'} aria-label="Resultados del catálogo">{catalog.data.map((card) => { const owned = entries.filter((e) => e.card_id === card.id && e.language === language).reduce((n, e) => n + e.quantity, 0); return <button className="card-tile" key={card.id} onClick={() => setSelection({ card, language })}><div className="card-art">{owned > 0 && <span className="owned-badge">En tu colección · {owned}</span>}<CardImage card={card} /></div><div className="card-info"><span className="card-code">{card.id} · {language.toUpperCase()}</span><h3>{card.name}</h3><div className="card-tile-footer"><span>Ver carta y precio</span><span className="add-circle"><Plus size={17} /></span></div></div></button> })}</div>}
