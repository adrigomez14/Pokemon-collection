...
const itemSchema = z.object({
  ...
  card_snapshot: snapshotSchema, created_at: z.iso.datetime({ offset: true }),
    ...
    for (const table of ['wishlists', 'wishlist_items'] as const) {
      for (let offset = 0; ; offset += 500) {
        ...
        let query = client.from(table).select(table === 'wishlists'
          ? 'id,user_id,name,created_at' : 'list_id,user_id,card_id,language,card_snapshot,created_at').eq('user_id', userId)
